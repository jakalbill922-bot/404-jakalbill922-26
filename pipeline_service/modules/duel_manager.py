...
class DuelManager:
    ...
    async def batch_render_candidates(self, ply_list: list[bytes], save_debug: bool = False) -> list[bytes]:
        """Batch render multiple PLY files. Returns list of rendered image bytes."""
        device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
        loop = asyncio.get_running_loop()
        
        logger.info(f"📸 Batch rendering {len(ply_list)} candidates...")
        start = time.time()
        ...
        if save_debug:
            for i, img_bytes in enumerate(rendered_images):
                if img_bytes:
                    ...
                    logger.info(f"💾 Saved candidate_{i+1}_combined_image.png")
        
        render_time = time.time() - start
        logger.success(f"✅ Rendered {len(ply_list)} candidates in {render_time:.2f}s ({render_time/len(ply_list):.2f}s per candidate)")
